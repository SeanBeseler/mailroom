...
def update_dic(donor_list, name, amount):
    ...
    temp_donor_list= {}
    flag = 0
    for dname in donor_list:
        if flag == 1 and dname != name:
            temp_donor_list[dname] = donor_list[dname]
        elif dname != name and donor_list.get(dname) >= donor_list.get(name):
            temp_donor_list[dname]= donor_list.get(dname)
        elif dname != name and donor_list.get(dname) < donor_list.get(name):
            temp_donor_list[name] = donor_list.get(name)
            flag = 1
            temp_donor_list[dname] = donor_list[dname]
        if flag == 0 and dname == name:
            temp_donor_list[dname] = donor_list[dname]

            
    return temp_donor_list
